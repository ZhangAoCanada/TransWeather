...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        attn = torch.nn.quantized.functional.linear(q[0, 0, ...], k.transpose(-2, -1)[0, 0, ...], scale = self.scale, zero_point = 0)
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        # x = (attn @ v).transpose(1, 2).reshape(B, N, C)
        x = torch.nn.quantized.functional.linear(attn[0, 0, ...], v[0, 0, ...], scale=1, zero_point=0).transpose(1, 2).reshape(B, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
...
class Transweather(nn.Module):
    ...
    def fuse_model(self):
        for m in self.modules():
            if type(m) == "EncoderTransformer":
                torch.quantization.fuse_modules(m.block1, [str(i) for i in range(len(m.block1))], inplace=True)
                torch.quantization.fuse_modules(m.patch_block1, [str(i) for i in range(len(m.patch_block1))], inplace=True)
                torch.quantization.fuse_modules(m.block2, [str(i) for i in range(len(m.block2))], inplace=True)
                torch.quantization.fuse_modules(m.patch_block2, [str(i) for i in range(len(m.patch_block2))], inplace=True)
                torch.quantization.fuse_modules(m.block3, [str(i) for i in range(len(m.block3))], inplace=True)
                torch.quantization.fuse_modules(m.patch_block3, [str(i) for i in range(len(m.patch_block3))], inplace=True)
                torch.quantization.fuse_modules(m.block4, [str(i) for i in range(len(m.block4))], inplace=True)
            if type(m) == "DecoderTransformer":
                torch.quantization.fuse_modules(m.block1, [str(i) for i in range(len(m.block1))], inplace=True)
