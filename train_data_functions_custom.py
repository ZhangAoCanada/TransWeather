...
class TrainData(data.Dataset):
    ...
    def pre(self, input_img):
        ...
        input_im = input_img.astype(np.float32) / 255.
    ...
    def get_images(self, index):
        ...
        gt_img = cv2.resize(gt_img, (960, 540))
        input_im = self.pre(input_img)
        gt = self.pre(gt_img)
        ...
        # input_crop_img = cv2.cvtColor(input_crop_img, cv2.COLOR_BGR2RGB)
        # input_crop_img = input_crop_img.astype(np.float32)
        # gt_crop_img = np.array(gt_crop_img)
        # gt_crop_img = cv2.cvtColor(gt_crop_img, cv2.COLOR_BGR2RGB)
        # gt_crop_img = gt_crop_img.astype(np.float32)
        # input_im = torch.from_numpy(input_crop_img)
        # gt = torch.from_numpy(gt_crop_img)
